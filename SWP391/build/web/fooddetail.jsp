    ...
    <div class="wrapper">
        ...
        <c:forEach items="${requestScope.infofood}" var="c">
            <div class="product-detail">
                ...
                <!-- Information -->
                <form method="post" action="detail" class="product-information col-sm-6" >
                    <input style="display:none" name="num" value="${requestScope.num}"></input>
                    <!-- Header -->
                    <h1>${c.name_food}</h1>
                    <!-- Description -->
                    ...
                    <input style="display: none" name="foodId" value="${c.id}"/>
                    <!-- Unit -->
                    <div class="form-outline">
                        <input type="number" name="quantity" class="form-control" min="1" value="1" step="1"/>
                    </div>          
                    <!-- Button Add -->
                    <br />
                    <button class="btn">Đặt mua</button>
                    <br />
                    <button class="btn" type="submit" id="addCartBtn">Thêm vào giỏ hàng</button>
                </form>
            </div>
        </c:forEach>
        ...
        <div id="footer">
         ...
         <%
                Logger logger = Logger.getLogger("ShowDetail"); // Đặt tên cho logger, ví dụ "FoodDetailServlet"

            Boolean showSuccessMessage = (Boolean) request.getSession().getAttribute("showSuccessMessage");
            if (showSuccessMessage != null && showSuccessMessage) {
                    logger.info("Thêm giỏ hàng thành công");
                out.println("<script>alert('Thêm giỏ hàng thành công');</script>");
                request.getSession().removeAttribute("showSuccessMessage");
            }
        %>
    <!-- MDB -->
    <script
...
//$('#addCartBtn').click(
//            function(foodId, quantity) {
//                var quantity = document.getElementById("quantity").value;
//                                                                $.ajax({
//                                                                    url: 'DetailController', // URL của servlet
//                                                                    type: "POST", // Phương thức POST để gửi yêu cầu xóa
//                                                                    data: {foodId: foodId, quantity: quantity}, // Tham số truyền đi
//                                                                    success: function (response) {
//                                                                        window.alert("Them gio hang thanh cong!!");
//                                                                    },
